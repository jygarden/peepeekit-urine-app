// api/transform.js
// Vercel Serverless Function — 건강어때 캐릭터 도감
// 컨셉: 우리 아이를 주토피아처럼 옷 입고 두 발로 서 있는 캐릭터로
//   1) 펫 정체성 보존 (종·털색·귀·눈)
//   2) Anthropomorphic — 두 발 + 옷 + 사람 같은 활동
//   3) MBTI 성격이 의상·포즈·배경에 반영
//   4) 캐리커처로 시그니처 특징 과장
...
// ─────────────────────────────────────────────
// MBTI별 시각적 단서 (의상 / 포즈 / 활동 / 배경 / 표정)
// 사람 직업·취미 컨셉을 동물 캐릭터에 입힘
// ─────────────────────────────────────────────
const VISUAL_CUES = {
  ENFJ: { outfit:'cozy beige cardigan over a soft white shirt, with a small notebook tucked under one arm', pose:'standing upright with one paw warmly outstretched, head slightly tilted in caring attention', activity:'listening empathetically to someone offscreen', bg:'warm sunlit counseling room with plants and soft pastel walls', expr:'gentle warm eyes, soft caring smile' },
...
Keep it tasteful (no offensive content) — just one charming "ha!" moment.
