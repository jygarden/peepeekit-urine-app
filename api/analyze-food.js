...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
    body{background:#F7F7F5;font-family:'Gowun Dodum',ui-rounded,'SF Pro Rounded',-apple-system,BlinkMacSystemFont,'Apple SD Gothic Neo','Noto Sans KR',sans-serif;padding-bottom:0;color:#1D1D1F;-webkit-font-smoothing:antialiased}
    ...
    #screenWelcome{padding:0;display:none;position:relative;z-index:2}
    ...
    .welcome-wrap{
      ...
      padding:60px 30px 60px;position:relative;z-index:3;text-align:center
    }
    .welcome-brand{
      font-size:15px;font-weight:700;color:#5E7CE2;letter-spacing:-.2px;
      opacity:0;animation:fadeSlideUp .7s cubic-bezier(.2,.8,.2,1) .15s forwards
    }
    .welcome-content{
      margin-top:90px;display:flex;flex-direction:column;align-items:center;width:100%;max-width:340px
    }
    .welcome-greet{
    ...
    .home-wrap{padding:70px 26px 0;min-height:100vh;display:flex;flex-direction:column}
    ...
    /* 오늘 비교 박스 */
    .today-compare-box{background:linear-gradient(135deg,#EFF6FF,#DBEAFE);border:1.5px solid #93C5FD;border-radius:14px;padding:14px 16px;margin-bottom:12px;font-size:12.5px;color:#1E40AF;line-height:1.6}
    .today-compare-box::before{content:"💬 ";font-size:14px}
    /* 라이트 팁 박스 */
    .light-tips-box{background:linear-gradient(135deg,#FAF5FF,#F3E8FF);border:1.5px solid #C4B5FD;border-radius:14px;padding:14px 16px;margin-bottom:12px}
    ...
    .report-summary-card{background:linear-gradient(135deg,#FEF3C7,#FCD34D);border-radius:18px;padding:18px;color:#78350F;margin-bottom:14px;text-align:center}
    ...
    .points-badge{display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#FEF3C7,#FBBF24);border:none;border-radius:999px;padding:8px 14px 8px 11px;cursor:pointer;box-shadow:0 2px 10px rgba(251,191,36,.30);font-family:inherit;color:#78350F;font-size:12.5px;font-weight:800;letter-spacing:-.2px;transition:transform .15s,box-shadow .2s}
...
<div id="app">
    ...
    <div class="welcome-wrap">
      ...
      <div class="welcome-content">
        ...
        <div id="welcomeFirst" style="display:none">
          ...
          <div class="welcome-sub">이름만 입력하면<br>바로 시작할 수 있어요</div>
          <div class="welcome-input-wrap">
            ...
            <div class="welcome-divider">또는</div>
            <input type="text" class="welcome-input" id="nameInput" placeholder="이름으로 시작 (예: 진영)" maxlength="10"
                   onkeydown="if(event.key==='Enter')saveName()"/>
            <button class="welcome-btn-secondary" onclick="saveName()">이름으로 시작하기</button>
            <div class="welcome-hint">
              <svg viewBox="0 0 24 24"><path d="M7 11 V 8 C 7 5, 9 3, 12 3 C 15 3, 17 5, 17 8 V 11"/><rect x="5" y="11" width="14" height="10" rx="2"/></svg>
              개인정보는 안전하게 보호돼요
            </div>
          </div>
        </div>
    ...
    <div class="home-wrap">
      ...
      <div class="home-top">
        <div class="home-title">
          <h1>오늘, 작은 신호를<br>확인해보세요</h1>
          <div class="sub">사람과 반려동물의 건강 지표를 1분 만에 기록해요</div>
        </div>
        <button class="bell-btn" onclick="openSettings()" aria-label="설정">
          <svg viewBox="0 0 24 24">
            <path d="M6 8 C 6 5, 8.5 3, 12 3 C 15.5 3, 18 5, 18 8 V13 L 20 17 H 4 L 6 13 V8 Z"/>
  ...
  <div class="screen" id="screenFoodCapture">
    <div class="section-title">🍽 식단 분석</div>
    <div class="card">
      <div class="field">
    ...
    <button class="upload-btn accent" style="border-color:#F59E0B" onclick="document.getElementById('foodCameraInput').click()">
      <div class="icon">📷</div>
      <div class="lbl" style="color:#F59E0B">카메라로 촬영</div>
      <div class="sub">지금 바로 식사 사진을 찍어주세요</div>
    </button>
    <button class="upload-btn" onclick="document.getElementById('foodGalleryInput').click()">
  ...
  <div class="screen" id="screenReport">
    <div class="report-wrap">
      ...
      <div style="font-size:21px;font-weight:800;color:#1D1D1F;margin-bottom:6px">📊 종합 영양 리포트</div>
  ...
  <div class="screen" id="screenFoodResult">
    <div id="foodResultCapture">
      ...
      <div class="env-health-box" style="background:linear-gradient(135deg,#fef3c7,#fde68a);border-color:#fbbf24" id="foodMissingBox">
        <div class="etitle" style="color:#92400E">⚖️ 영양소 균형</div>
        <div class="env-health-item" id="foodMissingItem" style="display:none">
          <div class="elabel" style="color:#92400E">📉 더 필요한 영양소</div>
  ...
  <div class="screen" id="screenCapture">
    ...
    <button class="upload-btn accent" onclick="document.getElementById('cameraInput').click()">
      <div class="icon">📷</div>
...
<div class="settings-overlay" id="settingsOverlay" onclick="if(event.target===this)closeSettings()">
  <div class="settings-modal">
    ...
    <button class="settings-btn settings-btn-secondary" style="color:#6366f1" onclick="manageProfilesFromSettings()">👨‍👩‍👧 프로필 관리 (가족·반려동물)</button>
    <button class="settings-btn settings-btn-danger" onclick="resetAllData()">모든 기록·이름 초기화</button>
  </div>
</div>

<input type="file" id="foodCameraInput" accept="image/*" capture="environment" style="display:none" onchange="handleFoodFile(this)"/>
<input type="file" id="foodGalleryInput" accept="image/*" style="display:none" onchange="handleFoodFile(this)"/>
...
function showScreen(id) {
  ...
  // 홈 진입 시 카드 정보·최근 기록 갱신 + 리마인더 체크
  if (id === 'screenHome') { updateHomeStats(); refreshPointsBadge(); checkReminder(); }
  // 촬영 화면 첫 진입 시 팁 표시
...
async function initWelcome() {
  ...
  if (came) return; // 처리 완료, 인사 → 홈 자동 전환됨
...
function handleFoodFile(input) {
  ...
  reader.onload = function(e) {
    foodImageSrc = e.target.result;
    foodImageB64 = e.target.result.split(',')[1];
    document.getElementById('foodPreviewImg').src = foodImageSrc;
...
async function confirmAndAnalyze() {
  ...
  try {
    const res = await fetch('/api/analyze-food', {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        imageB64: foodImageB64, mealTime: selectedMealTime, userProfile,
...
function renderFoodResult(r) {
  ...
  const items = (r.foods || []).map(f => {
    const nutrients = (f.keyNutrients || []).map(n =>
      `<div style="margin-top:4px"><strong>${n.name}</strong> — ${n.benefit}</div>`
    ).join('');
    return `<div class="food-detail-card">
      <div class="food-detail-name">
        <div><span style="font-weight:800">${f.name}</span> <span class="food-detail-amount">${f.amount || ''}</span></div>
        <div class="food-detail-cal">${(f.calories || 0).toLocaleString()} kcal</div>
      </div>
      ${nutrients ? `<div class="key-nutrient-row">⭐ <strong>핵심 영양소</strong>${nutrients}</div>` : ''}
    </div>`;
  }).join('');
  ...
  if (r.nextMeal && typeof r.nextMeal === 'object') {
    nextMealHtml = `<div class="next-meal-cta">
      <div class="next-meal-title">🍱 다음 식사 추천</div>
      <div class="next-meal-menu">${r.nextMeal.menu || ''}</div>
      <div class="next-meal-reason">${r.nextMeal.reason || ''}</div>
    </div>`;
  } else if (r.nextMeal) {
    nextMealHtml = `<div class="next-meal-cta">
      <div class="next-meal-title">🍱 다음 식사 추천</div>
      <div class="next-meal-menu">${r.nextMeal}</div>
    </div>`;
  }
  // 음식 궁합 박스
  let pairingsHtml = '';
  if (r.pairings && (r.pairings.good?.length || r.pairings.warning?.length)) {
  ...
  if (r.lightTips && r.lightTips.length > 0) {
    lightTipsHtml = `<div class="light-tips-box">
      ...
      ${r.lightTips.map(t => `<div class="light-tip-item"><span>${t}</span></div>`).join('')}
    </div>`;
  }
  ...
  (r.foods || []).forEach(f => {
    (f.keyNutrients || []).forEach(n => {
      if (!nutrientMap[n.name]) nutrientMap[n.name] = { benefit: n.benefit, foods: [] };
      nutrientMap[n.name].foods.push(f.name);
    });
  });
  let nutrientDictHtml = '';
  if (Object.keys(nutrientMap).length > 0) {
    nutrientDictHtml = `<div class="pairings-box" style="background:linear-gradient(135deg,#FFFBEB,#FEF3C7);border-color:#FBBF24">
      <div class="pairings-title" style="color:#92400E">⭐ 이 식사의 핵심 영양소</div>
      ${Object.entries(nutrientMap).map(([name, d]) => `
        <div class="pair-item" style="color:#78350F;background:rgba(255,255,255,.8)">
          <div class="pair-combo">${name}</div>
          <div class="pair-reason" style="margin-bottom:3px">${d.benefit}</div>
          <div style="font-size:10.5px;color:#A16207;font-weight:600">📍 ${d.foods.join(', ')}에서</div>
        </div>
      `).join('')}
    </div>`;
  }
  ...
  const baseHighlights = (r.highlights || []).map(h => {
    const ic = h.type === 'good' ? '✅' : h.type === 'warning' ? '⚠️' : '🚨';
    return `<div class="highlight-item ${h.type || 'good'}">${ic} ${h.text}</div>`;
  }).join('');
  container.innerHTML = baseHighlights + todayHtml + nutrientDictHtml + pairingsHtml + lightTipsHtml + nextMealHtml;
  // 영양소 막대 (대략적 기준치 기반)
  const n = r.nutrition || {};
  const targets = { carbs:300, protein:60, fat:65, fiber:25, sodium:2000, sugar:50 };
  const labels = { carbs:'탄수화물', protein:'단백질', fat:'지방', fiber:'식이섬유', sodium:'나트륨', sugar:'당류' };
  const units = { carbs:'g', protein:'g', fat:'g', fiber:'g', sodium:'mg', sugar:'g' };
  const nutriHtml = Object.keys(targets).map(k => {
    const val = n[k] || 0;
    const pct = Math.min(100, Math.round(val / targets[k] * 100));
    return `<div class="nutri-item">
      <div class="nutri-row">
        <span class="nutri-name">${labels[k]}</span>
        <span class="nutri-val">${val}${units[k]} <span style="color:#94A3B8;font-weight:500">/ ${targets[k]}${units[k]}</span></span>
      </div>
      <div class="nutri-bar"><div class="nutri-fill ${k}" style="width:${pct}%"></div></div>
    </div>`;
  }).join('');
  ...
  // 다음 식사
  if (r.nextMeal) {
    document.getElementById('foodNextMeal').textContent = r.nextMeal;
    document.getElementById('foodNextMealBox').style.display = 'block';
  } else { document.getElementById('foodNextMealBox').style.display = 'none'; }
  // 영양제
  ...
  if (r.tips && r.tips.length > 0) {
    document.getElementById('foodTipsList').innerHTML = r.tips.map(t =>
      `<div class="tip-row"><span class="tip-check">✓</span><div class="tip-text">${t}</div></div>`).join('');
    document.getElementById('foodTipsBox').style.display = 'block';
  } else { document.getElementById('foodTipsBox').style.display = 'none'; }
...
function renderReport() {
  ...
  try {
    ...
    const list = all.filter(h => new Date(h.date).getTime() >= cutoff);
    if (list.length === 0) {
    ...
    // 누적 통계
    const totalCal = list.reduce((s,h) => s + (h.totalCalories || 0), 0);
    const avgScore = Math.round(list.reduce((s,h) => s + (h.balanceScore || 0), 0) / list.length);
    const totalNutri = { carbs:0, protein:0, fat:0, fiber:0, sodium:0, sugar:0 };
    list.forEach(h => {
      if (h.nutrition) Object.keys(totalNutri).forEach(k => totalNutri[k] += (h.nutrition[k] || 0));
    });
    // 부족·과다 영양소 빈도
    ...
    // 권장량 비교 (오늘 vs 일주일)
    const target_targets = reportTab === 'today'
      ? { carbs:300, protein:60, fat:65, fiber:25, sodium:2000, sugar:50 }
      : { carbs:2100, protein:420, fat:455, fiber:175, sodium:14000, sugar:350 };
    const nutriHtml = Object.keys(target_targets).map(k => {
      const val = totalNutri[k];
      const tg = target_targets[k];
      const pct = Math.min(100, Math.round(val / tg * 100));
      const labels = { carbs:'탄수화물', protein:'단백질', fat:'지방', fiber:'식이섬유', sodium:'나트륨', sugar:'당류' };
      const units = { carbs:'g', protein:'g', fat:'g', fiber:'g', sodium:'mg', sugar:'g' };
      const cls = pct >= 90 ? '#10B981' : pct >= 50 ? '#6366F1' : '#EF4444';
      return `<div style="margin-bottom:8px">
        <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px">
          <span>${labels[k]}</span>
          <span style="font-weight:700">${val}${units[k]} <span style="color:#94A3B8">/ ${tg}${units[k]} (${pct}%)</span></span>
        </div>
        <div style="height:5px;background:#F1F5F9;border-radius:3px;overflow:hidden">
          <div style="width:${pct}%;height:100%;background:${cls};border-radius:3px"></div>
        </div>
      </div>`;
    }).join('');
    target.innerHTML = `
      ...
      <div class="report-section" style="background:linear-gradient(135deg,#F5F3FF,#EDE9FE);border:1.5px solid #C4B5FD">
        <div class="report-section-title" style="color:#5B21B6">💊 추천 영양제 (자주 부족한 거 기반)</div>
        <div style="font-size:12px;color:#4C1D95;line-height:1.7">
          ${topMissing.slice(0, 3).map(([n]) => {
            const map = {
              '비타민C': '비타민C 1000mg — 항산화·면역 강화',
              '칼슘': '칼슘+비타민D — 뼈 건강',
              '오메가3': '오메가3 EPA+DHA 1000mg — 혈관·두뇌',
              '식이섬유': '차전자피 분말 — 장 건강·포만감',
              '단백질': '유청 단백질 25g — 근육·회복',
              '철분': '철분 18mg + 비타민C — 빈혈 예방',
              '비타민D': '비타민D3 2000IU — 면역·뼈 건강'
            };
            return `<div style="margin-bottom:4px">✓ ${map[n] || `${n} 보충제`}</div>`;
          }).join('') || '<div>현재 부족한 영양소 없음 ✨</div>'}
        </div>
      </div>
      <div class="report-section" style="background:linear-gradient(135deg,#ECFDF5,#D1FAE5);border:1.5px solid #6EE7B7">
        <div class="report-section-title" style="color:#065F46">🏃 추천 운동</div>
...
async function downloadFoodResult() {
  try {
    const el = document.getElementById('foodResultCapture');
    const canvas = await html2canvas(el, { scale: 2, useCORS: true, backgroundColor: '#FAFCFF' });
    const link = document.createElement('a');
    link.download = `식단분석_${selectedMealTime}_${new Date().toLocaleDateString('ko-KR').replace(/\./g,'').replace(/ /g,'')}.png`;
...
async function downloadResult() {
  ...
  try {
    const el = document.getElementById('resultCapture');
    const canvas = await html2canvas(el, {
      scale: 2, useCORS: true, backgroundColor: '#f1f5f9', scrollY: -window.scrollY
    });
    const link = document.createElement('a');
    const baseName = subjectMode === 'human'
      ? (document.getElementById('humanName').value.trim() || '본인')
      : (document.getElementById('petName').value.trim() || '반려동물');
    link.download = `${baseName}_소변검사_${new Date().toLocaleDateString('ko-KR').replace(/\./g,'').replace(/ /g,'')}.png`;
    link.href = canvas.toDataURL('image/png');
    link.click();
  } catch(e) {
    alert('저장 중 오류가 발생했습니다. 다시 시도해주세요.');
  } finally {
    btn.textContent = '📥 결과 이미지 저장';
    btn.disabled = false;
  }
}
...
function resetAllData() {
  ...
  closeSettings();
  alert('모든 데이터가 삭제되었어요. 처음 화면으로 돌아갑니다.');
  location.reload();
}
